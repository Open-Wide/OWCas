...
class eZCASSSOHandler {
     ...
     */
    static function importLDAPUser( $login, $authenticationMatch = false ) {
        $password = md5( uniqid( $login, true ) );
        ...
        if( $databaseName === 'mysql' ) {
            $query = "SELECT contentobject_id, password_hash, password_hash_type, email, login
                      ...
                      WHERE ( $loginText ) AND
                            ...
                            ( ezcontentobject.id=contentobject_id OR ( password_hash_type=4 AND ( $loginText ) ) )";
        } else {
            $query = "SELECT contentobject_id, password_hash, password_hash_type, email, login
                      FROM ezuser, ezcontentobject
                      WHERE ( $loginText ) AND
                ...
                if( $hashType == eZUser::PASSWORD_HASH_MYSQL and $databaseName === 'mysql' ) {
                    $queryMysqlUser = "SELECT contentobject_id, password_hash, password_hash_type, email, login
                                       ...
                                       WHERE ezcontentobject.status='$contentObjectStatus' AND
                                             password_hash_type=4 AND ( $loginText )";
        ...
        if( $exists and $isEnabled and $canLogin ) {
            ...
            if( $ds ) {
                ...
                if( $LDAPGroupMappingType == $ByMemberAttribute or $LDAPGroupMappingType == $ByMemberAttributeHierarhicaly ) {
                    ...
                    $groupsTree[$userData['dn']] = array(
                        'data' => &$userData,
                        'parents' => array( ),
                        'children' => array( )
                    );

                    self::getUserGroupsTree( $requiredParams, $newfilter, $userData['dn'], $groupsTree, $stack, $groupSearchingDepth );
                ...
                $existingUser = self::publishUpdateUser( $extraNodeAssignments, $defaultUserPlacement, $userAttributes, $isUtf8Encoding );
                ldap_close( $ds );
                return $existingUser;
            } else {
    ...
    static function publishUpdateUser( $parentNodeIDs, $defaultUserPlacement, $userAttributes, $isUtf8Encoding = false ) {
        ...
        if( (!is_array( $parentNodeIDs ) or count( $parentNodeIDs ) < 1) and !is_numeric( $defaultUserPlacement ) ) {
            eZDebug::writeWarning( 'No one parent node IDs was passed for publishing new user (login = "' . $userAttributes['login'] . '")', __METHOD__ );
            return false;
        }
        ...
        if( $createNewUser ) {
            ...
            eZDebug::writeNotice( var_export( $operationResult, true ), __METHOD__ );
        } else {
            if( $userDataChanged ) {
                // Publish object
                $operationResult = eZOperationHandler::execute( 'content', 'publish', array(
    ...
    static function goAndPublishGroups( &$requiredParams, $curDN, &$groupsTree, &$stack, $depth, $isUser = false ) {
        ...
        if( count( $parentNodesForNew ) > 0 ) {
            if( $isUser ) {
                ...
                $newNodeIDs = self::publishNewUserGroup( $parentNodesForNew, array(
                    'name' => $currentName,
                    'description' => ''
                ) );
            }
     ...
     */
    static function getUserGroupsTree( &$requiredParams, $filter, $curDN, &$groupsTree, &$stack, // stack for recursion checking
    $depth = 0 ) {
        if( $depth == 0 ) {
            return false;
        }
        ...
        if( is_array( $entries ) and isset( $entries['count'] ) and $entries['count'] > 0 ) {
            ...
            for( $i = 0; $i < $entries['count']; $i++ ) {
                ...
                $ret = self::getUserGroupsTree( $requiredParams, $newfilter1, $parentDN, $groupsTree, $stack, $depth - 1 );
                if( isset( $groupsTree['_recursion_detected_'] ) and $groupsTree['_recursion_detected_'] ) {
                    return false;
                }
            }
     ...
     */
    static function getNodeAssignmentsForGroupName( $groupName, $isFirstGroupAssignment, &$defaultUserPlacement, &$extraNodeAssignments, $createMissingGroups, $parentNodeID ) {
        ...
        if( count( $newNodeIDs ) > 0 and $isFirstGroupAssignment ) {
            $defaultUserPlacement = $newNodeIDs[0];
            // We only supplied one parent to publishNewUserGroup(), so there is only one node
        } else if( count( $newNodeIDs ) > 0 ) {
            $extraNodeAssignments[] = $newNodeIDs[0];
            // We only supplied one parent to publishNewUserGroup(), so there is only one node
        }
    }
